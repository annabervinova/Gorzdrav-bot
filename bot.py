...
def api_get(path):
    ...
    try:
        ...
        log.info("GET %s → HTTP %s, %d байт", url, r.status_code, len(r.content))
        if r.status_code != 200:
            log.error("❌ Не 200! Тело ответа (первые 300 символов): %s",
                      r.text[:300].replace('\n', ' '))
            return []
        data = r.json()
    except requests.exceptions.Timeout:
        log.error("⏱ API timeout %s (запрос дольше 20 сек)", url)
        return []
    except requests.exceptions.SSLError as e:
        log.error("🔒 SSL error %s: %s", url, e)
        return []
    except requests.exceptions.ConnectionError as e:
        log.error("🔌 Connection error %s: %s", url, e)
        return []
    except Exception as e:
        log.exception("❓ Неизвестная ошибка запроса %s: %s", url, e)
        return []
    ...
    if isinstance(data, dict):
        if 'success' in data and not data.get('success'):
            log.warning("API %s success=false: %s", url, str(data)[:300])
            return []
